...
const ExtendedObjectACL = artifacts.require('ExtendedObjectACL')
...
contract('ExtendedObjectACL ', accounts => {
    let daoFact
    let acl
    let kernel
    let kernelBase
    ...
    beforeEach(async () => {
        ...
        const daclReceipt = await kernel.newAppInstance(await helper.apmNamehash("object-acl"), (await ExtendedObjectACL.new()).address, { from: holder })        
        objectACL = ExtendedObjectACL.at(daclReceipt.logs.filter(l => l.event == 'NewAppProxy')[0].args.proxy)
    ...
    describe('getObjectPermissionEntities', async () => {
        it('', async () => {
            await objectACL.createObjectPermission(root, 1, DUMMY_ROLE, root)
            const entities = await objectACL.getObjectPermissionEntities.call(1, DUMMY_ROLE)
    ...
    xdescribe('createObjectPermission', async () => {
